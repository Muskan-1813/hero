...
export default function FeatureCard({ title, desc, type }: Props) {
  ...
  return (
    <div
      className="relative rounded-[32px] p-8 h-[290px] w-[500px] overflow-hidden group border border-white/5 bg-[linear-gradient(180deg,#0e1117_0%,#0a0d12_100%)] shadow-[inset_0_1px_0_rgba(255,255,255,0.04)] cursor-pointer"
    ...
    >
      ...
      <div className="absolute bottom-5 left-6 right-6 h-40 pointer-events-none">
        ...
        {/* ================= PLANET ================= */}
        {type === "planet" && (
          <motion.div animate={isHovered ? "hover" : "rest"} className="relative w-[350px] h-full">
            <div className="absolute bottom-0 left-0 w-40 h-40">
              <Image src="/planet.png" alt="" fill className="object-contain" />
              <motion.div
                variants={{
                  rest: { rotate: 0 },
                  hover: { rotate: 360, transition: { repeat: Infinity, duration: 22, ease: "linear" } },
                }}
                className="absolute inset-0"
              >
                <Image src="/dashed-lines.png" alt="" fill className="object-contain opacity-80" />
              </motion.div>
            </div>
            <div className="absolute bottom-3.5 left-[115px]">
              {[ { angle: -14, length: 280 }, { angle: -4, length: 280 }, { angle: 6, length: 280 }, { angle: 14, length: 290 } ].map((line, i) => (
                <div key={i} className="relative h-[2px] mb-6 origin-left" style={{ width: `${line.length}px`, transform: `rotate(${line.angle}deg)` }}>
                  <div className="w-full h-[2px] border-t border-dashed border-white/40" />
                  <motion.div
                    variants={{
                      rest: { x: 0, opacity: 0 },
                      hover: { x: line.length, opacity: [0, 1, 1, 0], transition: { repeat: Infinity, duration: line.length / 70, ease: "linear", delay: i * 0.6 } },
                    }}
    ...
    <div className="absolute top-18 -translate-y-1/2 right-[-130px] w-[220px] h-[220px] rounded-full border border-[#7CFF00]/30 bg-gradient-to-l from-[#7CFF00]/10 to-transparent shadow-[0_0_80px_rgba(124,255,0,0.15)]" />
        ...
        {type === "devices" && (
          <div className="relative w-full h-[220px] flex items-center justify-center overflow-hidden">
            ...
            >
              <Image src="/shield.png" alt="Shield" fill className="object-contain" />
            </motion.div>
          </div>
        )}
        ...
        {type === "split" && (
          <div className="relative w-72 h-72 mx-auto mb-20 flex items-center justify-center">
            <motion.div
              animate={isHovered ? { scale: 1.4, opacity: 0.15 } : { scale: 1, opacity: 0.2 }}
      ...
      <Image src="/lock.png" alt="Lock" fill className="object-contain" />
        ...
        {type === "kill" && (
          <div className="relative w-full h-40 mx-auto flex items-center justify-center">
            <motion.div animate={isHovered ? { x: -45 } : { x: 0 }} className="absolute left-[calc(50%-150px)] w-[150px] h-[3px] bg-[#B6FF00] rounded-full shadow-[0_0_15px_rgba(182,255,0,0.8)] z-20" />
            <motion.div animate={isHovered ? { x: 45 } : { x: 0 }} className="absolute right-[calc(50%-150px)] w-[150px] h-[3px] bg-[#B6FF00] rounded-full shadow-[0_0_15px_rgba(182,255,0,0.8)] z-20" />
            <motion.div
              animate={isHovered ? { opacity: 1, scale: 1, filter: "brightness(1.2) drop-shadow(0 0 25px rgba(182, 255, 0, 0.7))" } : { opacity: 0, scale: 1 }}
              transition={isHovered ? { opacity: { duration: 0.3, delay: 0.1 }, scale: { type: "spring", stiffness: 150, damping: 12, delay: 0.1 } } : { duration: 0.15 }}
